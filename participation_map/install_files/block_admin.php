...
class block_admin extends block_list {
	...
	/// Participation Map
        ...
        if ($course->id != SITEID) {
            if (has_capability('moodle/role:assign', $context)) {
                $this->content->items[]='<a href="'.$CFG->wwwroot.'/participation_map/index.php?course='.$course->id.'" target="_blank">Participation Map</a>';
                $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/mini_bushgrapher.gif" class="icon" alt="" />';
            } else if (get_overridable_roles($context, 'name', ROLENAME_ORIGINAL)) {
                $this->content->items[]='<a href="'.$CFG->wwwroot.'/participation_map/index.php?course='.$course->id.'" target="_blank">Participation Map</a>';
                $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/mini_bushgrapher.gif" class="icon" alt="" />';
            }
